...
- [x] 2.1 Implement a local VoxCPM bridge contract (CLI or HTTP sidecar) that accepts response text plus cloned-voice profile inputs and returns playable audio.
...
- [x] 4.2 Validate the disabled-by-default path, successful local playback path, invalid-profile path, and provider-failure fallback path.
